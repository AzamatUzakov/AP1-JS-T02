...
class Board {
    ...
    receiveAttack(x, y) {//функция для атаки кораблья
        ...
        const ship = row ? row[x] : undefined
...
class App {
    ...
    run() {//функция проццес создание игроков и заполнение короблями их досок, в целом упровление игрой
        ...
        if (playerType === 1) {
            ...
            let aiPlayer = new AIPlayer("AIPlayer", 5)
            ...
            if (hit) {
                ...
                if (opponent.board.ships.every(e => e.isSunk())) {
                    ...
                    player1Obj.board.ships.forEach(ship => {
                        console.log(
                            `${player1Obj.name} ${player1Obj.board.ships.length} ${ship.length} ${ship.coordinatesX},${ship.coordinatesY} ${ship.position}`
                        );
                    });

                    opponent.board.ships.forEach(ship => {
                        console.log(
                            `${opponent.name} ${opponent.board.ships.length} ${ship.length} ${ship.coordinatesX},${ship.coordinatesY} ${ship.position}`
                        );
                    });

                    console.log(`\nПобедил ${currentPlayer.name}! 🎯`);
                    console.log(`=====================\n`);
                    return;
                }
            } else {
