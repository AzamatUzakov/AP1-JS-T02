...
class Player {
    ...
    placeShips(shipName, length, isVertical, startPosition) {// вызов функции у экземпляра Board, для размещение корабля
    ...
    takeTurn(opponent) {//Функция принимает аргумент ввиде экземпляр игрока и возврощает кардинаты и сам объект
        let x = +prompt("Кординаты по X")
        let y = +prompt("Кординаты по Y")
...
let playerPrompt = prompt("Имя игрока и длина доски")

let split = playerPrompt.split(" ")

let player = new Player(split[0], +split[1])
